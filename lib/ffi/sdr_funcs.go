	...
	if keepUnsealed {
		...
		if _, err := os.Stat(filepath.Join(sectorPaths.Cache, proofpaths.TreeDName)); err != nil {
			if os.IsNotExist(err) {
				...
				if txPiece != nil && txPiece.Version == txcarlib.TxCarV2 {
					// tempUnsealed is not originate one
					if !checkTxCarV2IndexFile(*txPiece, sectorPaths.Unsealed) {
	...
	retryUnsealedMove:
		...
		if txPiece != nil && txPiece.Version == txcarlib.TxCarV2 {
			...
			if fsInfo.Size() != int64(ssize) {
				// tempUnsealed is not originate one
				if !checkTxCarV2IndexFile(*txPiece, tempUnsealed) {
			...
			{ // get index from unsealed
				...
				err = createTxCarV2IndexFileFromUnsealed(tempUnsealed, sector.ProofType, *txPiece, indexPath)
				if err != nil {
...
func createTxCarV2IndexFileFromUnsealed(unsealedPath string, proofType abi.RegisteredSealProof, txPiece txcarlib.TxPiece, indexPath string) error {
	var br io.Reader
	{
...
func checkTxCarV2IndexFile(txPiece txcarlib.TxPiece, indexPath string) bool {
	...
	if txPieceRec.Version != txcarlib.TxCarV2 {
		return false
	}
